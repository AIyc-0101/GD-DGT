...
class swA(nn.Module):
    def __init__(self, in_channels, k=40):
...
def save_attention_heatmap_ply(points, attn_map, filepath):

    N = points.shape[0]
    attn_map = attn_map.mean(0)  
    attn_weights = attn_map.mean(dim=0).numpy()  # shape: [N]

    attn_weights = (attn_weights - attn_weights.min()) / (attn_weights.max() - attn_weights.min() + 1e-8)

    reds = (attn_weights * 255).astype(np.uint8)
    greens = np.zeros_like(reds)
    blues = (255 - reds).astype(np.uint8)
    vertices = np.array(
        [(points[i, 0], points[i, 1], points[i, 2], reds[i], greens[i], blues[i]) for i in range(N)],
    ...
    print(f"Saved attention heatmap to {filepath}")


class TransformerEncoderLayer(nn.Module):
    def __init__(self, d_model=256, num_heads=4, dropout=0, ffn_expansion=2):
...
class DT_cls(nn.Module):

    def __init__(self, args, output_channels=40):
        super(DT_cls, self).__init__()
        self.args = args
        self.k = args.k
...
class DGCNN_partseg(nn.Module):
    def __init__(self, args, seg_num_all):
        ...
        self.conv5 = nn.Sequential(nn.Conv2d(64 * 2, 64, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv6 = nn.Sequential(nn.Conv1d(64, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn6,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv7 = nn.Sequential(nn.Conv1d(16, 64, kernel_size=1, bias=False),
                                   self.bn7,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv8 = nn.Sequential(nn.Conv1d(1024, 256, kernel_size=1, bias=False),
                                   self.bn8,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.dp1 = nn.Dropout(p=args.dropout)
        ...
        self.lwsa = swA(in_channels=64, k=40)
        self.fuse1 = MultiScaleFeatureFusion(channels_list=[64, 64, 64])
        self.fuse2 = MultiScaleFeatureFusion(channels_list=[1024, 64])
        self.fuse3 = MultiScaleFeatureFusion(channels_list=[1024, 64, 64, 64])
        self.attention = TransformerEncoderLayer(d_model=1024, num_heads=4)
    ...
    def forward(self, x, l):
        ...
        x= self.fuse1([x1, x2, x3]) # (batch_size, 64, num_points)

        x = self.conv6(x)  # (batch_size, 64, num_points) -> (batch_size, emb_dims, num_points)
        ...
        x = self.fuse3([x, x1, x2, x3])  # (batch_size, 1088, num_points)
        x = x.permute(0, 2, 1) 
        x = self.attention(x)
        x = x.permute(0, 2, 1)

        x = self.conv8(x)  # (batch_size, 1088, num_points) -> (batch_size, 256, num_points)
        x = self.dp1(x)
...
class DGCNN_semseg_s3dis(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fuse1(x1, x2, x3)    # (batch_size, 64*3, num_points)

        x = self.conv6(x)                       # (batch_size, 64*3, num_points) -> (batch_size, emb_dims, num_points)
        x = x.max(dim=-1, keepdim=True)[0]      # (batch_size, emb_dims, num_points) -> (batch_size, emb_dims, 1)

        x = x.repeat(1, 1, num_points)          # (batch_size, 1024, num_points)
        x = self.fuse3(x, x1, x2, x3)   # (batch_size, 1024+64*3, num_points)
